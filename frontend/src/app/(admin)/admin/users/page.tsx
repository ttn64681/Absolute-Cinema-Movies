...
function AdminUsersPage() {
  ...
  const handleDeleteUser = async (userId: number, userName: string) => {
    ...
    if (
      !confirm(
        `Are you sure you want to delete ${userName}? This will permanently delete all associated data including bookings, payment cards, and addresses.`
      )
    ) {
      return;
    }
              ...
              <div className="rounded-md overflow-hidden h-[60vh] overflow-y-auto bg-[#242424] border border-white/10">
                {memberList.length === 0 ? (
                  ...
                  memberList.map((user) => (
                    <div
                      key={user.id}
                      className="flex items-center justify-between px-6 py-4 border-b border-white/10 hover:bg-white/5 transition-colors"
                    >
                      <div className="flex-1 flex items-center gap-4 min-w-0">
                        <div className="flex flex-col min-w-0">
                          <span className="font-afacad text-white truncate">
