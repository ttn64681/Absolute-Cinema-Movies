...
/** Movie list item shape: AdminMovie plus optional display/showtime fields */
interface AdminMovieWithMeta extends AdminMovie {
  date?: string;
  time?: string;
  _meta?: {
...
/** Parse time string + ampm to minutes for sorting (e.g. "2:30" + "PM" -> 870) */
...
interface MoviesListProps {
  movies: AdminMovieWithMeta[];
  setMovies: Dispatch<SetStateAction<AdminMovieWithMeta[]>>;
}

export default function MoviesList({ movies, setMovies }: MoviesListProps) {
  const { adminMovies, pagination, goToNextPage, goToPreviousPage } = useAdminMoviesList();
  const moviesList = adminMovies ?? [];

  const [showModal, setShowModal] = useState(false);
  const [editingMovie, setEditingMovie] = useState<AdminMovieWithMeta | null>(null);
  const [showScheduleModal, setShowScheduleModal] = useState(false);
  const [schedulingMovie, setSchedulingMovie] = useState<AdminMovieWithMeta | null>(null);
  ...
  const openEditModal = (movie: AdminMovieWithMeta) => {
  ...
  const onMovieSaved = (savedMovie: AdminMovieWithMeta) => {
  ...
  const openScheduleModal = (movie: AdminMovieWithMeta) => {
    ...
    <div className="text-white pb-16" style={{ backgroundColor: '#1C1C1C', minHeight: '100vh' }}>
            ...
            {paginatedMovies.length === 0 ? (
              ...
              paginatedMovies.map((movie) => {
                ...
                const sorted = [...allShowtimes].sort((a, b) => {
                  const dateStrA = (a.date ?? '').split('/').reverse().join('-');
                  const dateStrB = (b.date ?? '').split('/').reverse().join('-');
                  const dateA = new Date(dateStrA);
                  const dateB = new Date(dateStrB);
                  if (dateA.getTime() !== dateB.getTime()) {
                    return dateA.getTime() - dateB.getTime();
                  }
                  const timeA = parseTime(a.time ?? '', a.ampm ?? 'AM') || 0;
                  const timeB = parseTime(b.time ?? '', b.ampm ?? 'AM') || 0;
                  return timeA - timeB;
                });
                ...
                return (
                  <li key={movie.movie_id} className="flex items-center py-3 sm:py-4">
                    <div className="flex-1 text-gray-200 font-afacad px-25 min-h-[1.5rem]">{movie.title}</div>
                    <div
...

      {
        <div className="max-w-[65rem] mx-auto mt-4 px-4 flex items-center justify-center gap-4">
          ...
          <span className="text-white font-afacad">Page {pagination.currentPage + 1}</span>
          <button
            type="button"
      ...
      <MovieFormModal
        ...
        initialMovieId={editingMovie?.movie_id ?? 0}
      />
